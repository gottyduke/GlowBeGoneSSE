...
namespace
{
	bool ForEachContainerEntry(RE::ContainerObject* a_entry)
	{
		...
		const auto weap = static_cast<RE::TESObjectWEAP*>(a_entry->obj);
		auto ench = weap->formEnchanting;
		if (!ench) {
			return true;
		}

		RE::EffectSetting* effectSetting = nullptr;
	...
	{
		const auto container = a_actor->GetContainer();
		if (container) {
			container->ForEachContainerObject(ForEachContainerEntry);
		}

		const auto changes = a_actor->GetInventoryChanges();
	...
	{
	public:
		using func_t = decltype(&Actor::LoadGame);
		...
		static void InstallHooks()
		{
			if (*Settings::removeWeaponFX) {
				...
				func = vTable.WriteVFunc(0xF, &Actor::LoadGame);
...
{
public:
	using func_t = decltype(&PlayerCharacter::LoadGame);
	static inline REL::Function<func_t> func;
	...
	static void InstallHooks()
	{
		if (*Settings::removeWeaponFX) {
			REL::Offset<std::uintptr_t> vTable(REL::ID(260538));
			func = vTable.WriteVFunc(0xF, &PlayerCharacter::LoadGame);
